...
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading .arena-title h1{'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading .arena-title .arena-mark,'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading .arena-live-logo{'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading .status-line{\n  display:none;'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) #stateBadge{'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .topline{'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .topline .brand{'));
assert(baseUi.includes("width:96px;"));
assert(baseUi.includes("right:0;\n  bottom:0;"));
assert(baseUi.includes("transform:none;"));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading .arena-title{width:215px;max-width:72%;margin:0 auto;transform:none}'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading{min-height:88px;margin:0 0 -2px}'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-heading .arena-live-logo{width:100%;height:auto;max-height:96px}'));
assert(baseUi.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"])[data-arena-view="arena"] #hero:is(.registration-mode,.live-dashboard)'));
assert(baseUi.includes('body[data-arena-phase="running"][data-arena-view="arena"] #hero.asset-panel-stats.live-dashboard:before{display:none}'));
...
assert(baseUi.includes('#hero:is(.registration-mode,.live-dashboard) .registration-inline{'));
assert(baseUi.includes('#hero:is(.registration-mode,.live-dashboard) .stats{'));
assert(baseUi.includes("if(!['registration','running'].includes(state.status))"));
assert(baseUi.includes("lock.textContent=state.status==='running'?'LIVE':(locked?'LOCKED':'OPEN')"));
...
assert(featurePackSource.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) #connectionChip{position:absolute;left:50%;bottom:0;z-index:20;margin:0;transform:translateX(-50%)}'));
...
assert(productPassSource.includes(".topline #hostTrigger.show"));
assert(productPassSource.includes("if(['registration','running'].includes(state.status))"));
assert(productPassSource.includes("content.insertBefore(heading,top)"));
assert(productPassSource.includes("hero.insertBefore(heading,hero.firstChild)"));
assert(productPassSource.includes("left:0;bottom:0;width:96px;min-width:96px;min-height:30px;padding:0 7px;font-size:7px"));
...
assert(prizePackSource.includes('body:is([data-arena-phase="registration"],[data-arena-phase="running"]) .arena-nav'));
assert(prizePackSource.includes("margin:-28px auto -82px"));
assert(prizePackSource.includes("margin:-26px -3% -78px"));
