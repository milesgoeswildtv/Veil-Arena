...
export function createGame({ guildId, channelId, hostId, themeId = "vibe_queen_slots" }) {
  return {
    id: crypto.randomUUID(), guildId, channelId, hostId, themeId,
    status: "registration", round: 0, players: {}, aliveIds: [], eliminatedIds: [],
    history: [], crowdVote: null, winnerId: null, createdAt: new Date().toISOString()
  };
}
...
export function addPlayer(game, player) {
  ...
  game.players[player.id] = {
    ...
    alive: true, eliminations: 0, revivals: 0, crowdPinsSurvived: 0
  };
...
export function castCrowdVote(game, spectatorId, playerId) {
  ...
  if (previous && vote.totals[previous] > 0) vote.totals[previous]--;
  vote.votesBySpectator[spectatorId] = playerId;
  vote.totals[playerId]++;
  return vote;
}
...
export function getCrowdQualifiers(totals) {
  ...
  const cutoff = entries[1][1];
  return entries.filter(([, votes]) => votes >= cutoff).map(([id]) => id);
}
...
export function resolveCrowdVote(game, rng = Math.random) {
  ...
  if (qualifiers.length < 2) {
    ...
    qualifiers.push(...uniqueRandom(remaining, 2 - qualifiers.length, rng));
  }
