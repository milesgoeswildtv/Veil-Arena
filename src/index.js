import { InteractionType,InteractionResponseType,verifyDiscordRequest,jsonResponse,interactionMessage,interactionUpdate,button,actionRow,userFromInteraction,registerGuildCommands,createChannelMessage,createChannelTextFile } from "./discord.js";
import { createGame,addPlayer,removePlayer,startGame,castCrowdVote } from "./core/engine.js";
import { ensureSchema,saveGame,loadGame,loadActiveGameForChannel,loadFinishedGameForGuild,getGuildConfig,setGuildTheme,loadPlayerStats,loadArenaLeaderboard } from "./storage.js";
import { getTheme } from "./themes/index.js";
import { rulesForTheme } from "./rules.js";
import { FULL_TILT_GUILD_ID,VIBE_QUEEN_GUILD_ID,themeForGuild } from "./server-config.js";
...
async function handleArenaCommand(interaction,env){
  ...
  if(sub==="rules"){const themeId=await themeIdForGuild(env,guildId);const posted=await createChannelMessage(channelId,env.DISCORD_BOT_TOKEN,{content:rulesForTheme(themeId)});await scheduleMessageDelete(env,channelId,posted?.id,30000,"discord");return interactionMessage("📜 Arena rules posted for **30 seconds**.",[],true);}
...
async function handleLeaderboardCommand(interaction,env){if(!interaction.guild_id)return interactionMessage("Arena leaderboards only exist inside a server.",[],true);if(!env.DB)return interactionMessage("Arena database is not configured yet.",[],true);await ensureSchema(env.DB);const b=await loadArenaLeaderboard(env.DB,interaction.guild_id,5);const content=["# 🏆 ARENA LEADERBOARD",boardSection("⚔️ MOST ARENAS PLAYED",b.gamesPlayed),boardSection("👑 MOST WINS",b.wins),boardSection("💀 TOTAL KILLS",b.kills),boardSection("🕯️ TOTAL REVIVALS",b.revivals),boardSection("🔥 MOST KILLS IN ONE ARENA",b.singleGameKills),boardSection("👁️ CROWD VOTES SURVIVED",b.crowdSurvivals)].join("\n\n");return interactionMessage(content);}
async function handleLogCommand(interaction,env){if(!interaction.guild_id)return interactionMessage("Arena logs only exist inside a server.",[],true);if(!env.DB)return interactionMessage("Arena database is not configured yet.",[],true);await ensureSchema(env.DB);const requested=Math.max(1,Number(interaction.data?.options?.find(x=>x.name==="match")?.value||1));const game=await loadFinishedGameForGuild(env.DB,interaction.guild_id,requested-1);if(!game)return interactionMessage(`I couldn't find completed Arena match #${requested} for this server.`,[],true);const log=buildArenaLog(game,requested),filename=`arena-match-${requested}-${game.id.slice(0,8)}.txt`;await createChannelTextFile(interaction.channel_id,env.DISCORD_BOT_TOKEN,filename,log,`📜 **Arena Match ${requested} narration log**`);return interactionMessage(`Done — I posted **Arena Match ${requested}** as a text file in this channel.\n\nUse \`/arenalog match:2\` for the previous match, \`match:3\` for the one before that, etc.`,[],true);}
async function handleComponent(interaction,env){const parts=(interaction.data?.custom_id||"").split(":"),action=parts[1],gameId=parts[2],user=userFromInteraction(interaction);if(!gameId||!user||!env.DB)return interactionMessage("Arena couldn't read that action.",[],true);await ensureSchema(env.DB);const game=await loadGame(env.DB,gameId);if(!game)return interactionMessage("That Arena game no longer exists.",[],true);if(action==="join"){try{addPlayer(game,user);await saveGame(env.DB,game);return interactionUpdate(registrationMessage(game),registrationComponents(game));}catch(e){return interactionMessage(e.message,[],true);}}if(action==="leave"){try{removePlayer(game,user.id);await saveGame(env.DB,game);return interactionUpdate(registrationMessage(game),registrationComponents(game));}catch(e){return interactionMessage(e.message,[],true);}}if(action==="start"){if(user.id!==game.hostId)return interactionMessage("Only the Arena host can start the game.",[],true);try{startGame(game);await saveGame(env.DB,game);await kickCoordinator(env,game.channelId,"kick","discord");const theme=getTheme(game.themeId);return interactionUpdate(`# ${theme.labels.arena}\nThe doors close. **${game.aliveIds.length} players** are inside.\n\nThe first round begins now.`,[]);}catch(e){return interactionMessage(e.message,[],true);}}if(action==="vote_open"){if(!game.crowdVote||game.crowdVote.status!=="open")return interactionMessage("The audience vote is closed.",[],true);if(game.aliveIds.includes(user.id))return interactionMessage("You're still fighting. The audience gets this vote.",[],true);const {rows,page,totalPages}=voteSelect(game,Number(parts[3]||0)),voteName=game.themeId==="full_tilt"?"Final Bet":"Final Scare";return interactionMessage(`**Choose who enters the ${voteName}.**\nPage ${page+1}/${totalPages}\nYou may change your vote until voting closes.`,rows,true);}if(action==="vote_cast"){if(!game.crowdVote||game.crowdVote.status!=="open")return interactionMessage("The audience vote is closed.",[],true);if(game.aliveIds.includes(user.id))return interactionMessage("You're still fighting. The audience gets this vote.",[],true);const id=interaction.data?.values?.[0];try{castCrowdVote(game,user.id,id);await saveGame(env.DB,game);return interactionMessage(`Vote locked on **${game.players[id]?.displayName||"Unknown"}**. You can change it before time runs out.`,[],true);}catch(e){return interactionMessage(e.message,[],true);}}return interactionMessage("Unknown Arena action.",[],true);}
async function handleDiscord(request,env){const raw=await request.text();if(!await verifyDiscordRequest(request,env.DISCORD_PUBLIC_KEY,raw))return new Response("Bad signature",{status:401});const i=JSON.parse(raw);if(i.type===InteractionType.PING)return jsonResponse({type:InteractionResponseType.PONG});if(i.type===InteractionType.APPLICATION_COMMAND){if(i.data?.name==="arena")return handleArenaCommand(i,env);if(i.data?.name==="arenastats")return handleStatsCommand(i,env);if(i.data?.name==="arenaleaderboard")return handleLeaderboardCommand(i,env);if(i.data?.name==="arenalog")return handleLogCommand(i,env);}if(i.type===InteractionType.MESSAGE_COMPONENT&&i.data?.custom_id?.startsWith("arena:"))return handleComponent(i,env);return interactionMessage("Veil doesn't know that Arena action yet.",[],true);}

function telegramScope(chatId){return`tg:${chatId}`;}
function telegramCommand(text=""){const first=String(text).trim().split(/\s+/)[0]||"";return first.toLowerCase().split("@")[0];}
function telegramArgs(text=""){return String(text).trim().split(/\s+/).slice(1);}
function telegramGroup(chat){return chat?.type==="group"||chat?.type==="supergroup";}
function telegramStatsText(user,stats){return statsText(user,stats).replace("in this server","in this Telegram group");}
function telegramLeaderboardText(b){return["# 🏆 DWALLET ARENA LEADERBOARD",boardSection("⚔️ MOST ARENAS PLAYED",b.gamesPlayed),boardSection("👑 MOST WINS",b.wins),boardSection("💀 TOTAL KILLS",b.kills),boardSection("🕯️ TOTAL REVIVALS",b.revivals),boardSection("🔥 MOST KILLS IN ONE ARENA",b.singleGameKills),boardSection("👁️ CROWD VOTES SURVIVED",b.crowdSurvivals)].join("\n\n");}

async function telegramStartArena(message,env){
  const chat=message.chat,user=userFromTelegram(message.from);
  if(!telegramGroup(chat))return sendTelegramMessage(String(chat.id),env.TELEGRAM_BOT_TOKEN,{text:"Arena can only run inside a Telegram group or supergroup."});
  if(!user)return null;
  await ensureSchema(env.DB);
