...
.connection-chip{display:inline-flex;align-items:center;gap:6px;margin-left:auto;margin-right:8px;padding:6px 9px;border:1px solid #49355e;border-radius:999px;background:#0a0710cc;color:#b8a9c3;font:850 8px/1 ui-monospace,monospace;letter-spacing:.1em}.connection-dot{width:7px;height:7px;border-radius:50%;background:#e3b64d;box-shadow:0 0 8px #e3b64d88}.connection-chip.live .connection-dot{background:#55e2ae;box-shadow:0 0 8px #55e2ae99}.connection-chip.offline .connection-dot{background:#ef526f;box-shadow:0 0 8px #ef526f99}.feature-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:14px}.feature-card{border:1px solid #3f2b4c;border-radius:14px;background:linear-gradient(155deg,#120d18e8,#09070deb);padding:14px;min-width:0}.feature-card summary{cursor:pointer;list-style:none;color:#d9c5e8;font:950 10px/1.2 ui-monospace,monospace;letter-spacing:.14em}.feature-card summary::-webkit-details-marker{display:none}.feature-body{margin-top:12px;color:#b9acc4;font-size:12px;line-height:1.45}.feature-row{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 0;border-bottom:1px solid #ffffff0b}.feature-row:last-child{border-bottom:0}.feature-actions{display:flex;flex-wrap:wrap;gap:7px;margin-top:10px}.feature-mini{appearance:none;border:1px solid #654676;border-radius:9px;background:#160e1d;color:#fff;padding:8px 10px;font-weight:900;font-size:11px}.feature-mini.primary{border-color:#a867e8;background:#472361}.feature-mini.danger{border-color:#8f2c3e;background:#341018}.feature-mini:disabled{opacity:.42}.ready-pill{padding:3px 6px;border-radius:999px;border:1px solid #4b3658;font:850 8px/1 ui-monospace,monospace;color:#9d91a5}.ready-pill.yes{border-color:#2f8067;color:#75e8bd}.final-five-strip{display:none;margin:0 0 12px;padding:10px 12px;border:1px solid #9b7338;border-radius:12px;background:linear-gradient(90deg,#2b1a0d,#1a0d22,#2b1a0d);color:#f2c968;text-align:center;font:1000 11px/1.2 ui-monospace,monospace;letter-spacing:.18em}.final-five-mode .final-five-strip{display:block}.final-five-mode .state-shell{box-shadow:0 26px 80px #000b,inset 0 0 0 1px #e3b64d44,0 0 34px #c58d3150}.vote-progress{grid-column:1/-1;display:grid;gap:6px;margin-top:4px}.vote-progress-row{display:grid;grid-template-columns:minmax(80px,1fr) 2fr auto;align-items:center;gap:8px;font-size:11px}.vote-progress-track{height:7px;border-radius:99px;background:#24172d;overflow:hidden}.vote-progress-fill{height:100%;background:linear-gradient(90deg,#8d57e8,#c45cff);border-radius:inherit}.recap-card{display:none}.recap-card.show{display:block}.recap-title{font-weight:1000;font-size:20px;color:#f2c968}.history-entry{padding:8px 0;border-bottom:1px solid #ffffff0b}.history-entry:last-child{border-bottom:0}.reaction-bar{display:none;position:sticky;bottom:8px;z-index:40;margin-top:12px;padding:8px;border:1px solid #553866;border-radius:14px;background:#09060dd9;backdrop-filter:blur(10px);grid-template-columns:repeat(6,1fr);gap:6px}.reaction-bar.show{display:grid}.reaction-button{border:1px solid #4b3559;border-radius:10px;background:#140d1a;color:#fff;font-size:21px;padding:8px}.reaction-layer{position:fixed;inset:0;pointer-events:none;z-index:998;overflow:hidden}.reaction-float{position:absolute;bottom:12%;font-size:30px;animation:reactionFloat 2.4s ease-out both;filter:drop-shadow(0 3px 8px #000)}@keyframes reactionFloat{0%{opacity:0;transform:translateY(30px) scale(.7)}15%{opacity:1}100%{opacity:0;transform:translateY(-58vh) translateX(var(--drift)) scale(1.25) rotate(var(--rot))}}.audio-range{width:100%;accent-color:#a45ee8}.diag-modal{display:none;position:fixed;inset:0;z-index:1200;background:#050308e8;padding:18px;place-items:center}.diag-modal.show{display:grid}.diag-box{width:min(560px,100%);max-height:80vh;overflow:auto;border:1px solid #6a477e;border-radius:18px;background:#0d0912;padding:18px}.diag-box pre{white-space:pre-wrap;overflow-wrap:anywhere;color:#cbbfd6;font:12px/1.55 ui-monospace,monospace}.host-tools{display:none}.host-tools.show{display:block}.cooldown-note{color:#e8c46e;font-size:11px;margin-top:8px}.share-code{font:950 13px/1 ui-monospace,monospace;letter-spacing:.18em;color:#d8b7f2}.paused-mark{color:#f2c968!important}.hall-list{display:grid;gap:6px}.hall-item{display:flex;justify-content:space-between;gap:10px;padding:8px;border:1px solid #352642;border-radius:10px;background:#0b080f}.utility-row{display:flex;flex-wrap:wrap;gap:7px;margin-top:10px}@media(max-width:760px){.feature-grid{grid-template-columns:1fr}.connection-chip{margin-left:0}.vote-progress-row{grid-template-columns:minmax(70px,1fr) 1.5fr auto}}
</style>`;
...
(() => {
  ...
  function installFeatureDom(){
    ...
    if(hero){
      const strip=document.createElement('div'); strip.id='finalFiveStrip'; strip.className='final-five-strip'; strip.textContent='FINAL FIVE // SPECIAL EVENTS LOCKED OUT';
      const title=hero.querySelector('.arena-title'); if(title) title.insertAdjacentElement('afterend',strip); else hero.prepend(strip);
    }
    ...
    if(content){
      content.insertAdjacentHTML('beforeend','<div class="feature-grid" id="featureGrid">'
        ...
        +'<section class="feature-card recap-card" id="recapPanel"><div class="recap-title">POST-GAME RECAP</div><div class="feature-body" id="recapBody"></div></section>'
        +'<section class="feature-card host-tools" id="hostTools"><div style="font:950 10px ui-monospace,monospace;letter-spacing:.14em;color:#d9c5e8">HOST CONTROL</div><div class="feature-body" id="hostBody"></div></section>'
        ...
        +'</div><div class="reaction-bar" id="reactionBar">'+['💀','🔥','😂','😈','👀','💜'].map(x=>'<button class="reaction-button" data-reaction="'+x+'">'+x+'</button>').join('')+'</div>');
    }
  ...
  function setConnection(mode){
    ...
    text.textContent=mode==='live'?'LIVE':mode==='offline'?'OFFLINE':'RECONNECTING';
  ...
  api = async function featureApi(path, body){
    ...
    try{ const result=await originalApi(path,body); setConnection('live'); return result; }
    catch(error){ setConnection(navigator.onLine===false?'offline':'syncing'); throw error; }
  };
  ...
  function renderVoteProgress(){
    const grid=document.getElementById('voteGrid'); if(!grid)return;
    grid.querySelector('.vote-progress')?.remove();
    if(!state?.crowdVote)return;
    const totals=state.crowdVote.totals||{}; const total=Object.values(totals).reduce((sum,value)=>sum+Number(value||0),0);
    const wrap=document.createElement('div'); wrap.className='vote-progress';
    wrap.innerHTML=(state.crowdVote.eligibleIds||[]).map(id=>{const p=(state.players||[]).find(x=>String(x.id)===String(id));const count=Number(totals[id]||0);const pct=total?Math.round(count*100/total):0;return '<div class="vote-progress-row"><span>'+featureEsc(p?.displayName||'Player')+'</span><span class="vote-progress-track"><span class="vote-progress-fill" style="display:block;width:'+pct+'%"></span></span><strong>'+count+'</strong></div>'}).join('');
  ...
  function renderRecap(){
    const panel=document.getElementById('recapPanel'),body=document.getElementById('recapBody'); if(!panel||!body)return;
    const r=state?.recap; panel.classList.toggle('show',Boolean(r)); if(!r)return;
    const kills=r.killLeaders?.length?featureEsc(r.killLeaders.join(', '))+' · '+r.maxKills+' KO':'No eliminations recorded';
    const revives=r.reviveLeaders?.length?featureEsc(r.reviveLeaders.join(', '))+' · '+r.maxRevives+' revival'+(r.maxRevives===1?'':'s'):'No revivals recorded';
    const crowd=r.crowdSurvivors?.length?r.crowdSurvivors.map(x=>featureEsc(x.displayName)+' · '+x.survived).join('<br>'):'No crowd-showdown survivors recorded';
    body.innerHTML='<div class="feature-row"><span>Champion</span><strong>'+featureEsc(r.winnerName)+'</strong></div><div class="feature-row"><span>Rounds</span><strong>'+r.rounds+'</strong></div><div class="feature-row"><span>Players</span><strong>'+r.playerCount+'</strong></div><div class="feature-row"><span>Top eliminations</span><strong>'+kills+'</strong></div><div class="feature-row"><span>Top revivals</span><strong>'+revives+'</strong></div><div class="feature-row"><span>Crowd survives</span><strong>'+crowd+'</strong></div>';
  }

  function renderHostTools(){
    const panel=document.getElementById('hostTools'),body=document.getElementById('hostBody'); if(!panel||!body)return;
    const show=Boolean(state?.viewer?.isHost&&state.status==='registration'); panel.classList.toggle('show',show); if(!show)return;
    const players=(state.players||[]).filter(p=>String(p.id)!==String(state.hostId));
    body.innerHTML='<div class="feature-row"><span>Ready check</span><strong>'+Number(state.readyCount||0)+' / '+Number(state.playerCount||0)+'</strong></div>'+(players.length?players.map(p=>'<div class="feature-row"><span>'+featureEsc(p.displayName)+(p.simulated?' · BOT':'')+'</span><span style="display:flex;align-items:center;gap:6px"><span class="ready-pill '+(p.ready?'yes':'')+'">'+(p.ready?'READY':'WAITING')+'</span>'+featureButton('REMOVE','remove','danger','data-target-id="'+featureEsc(p.id)+'"')+'</span></div>').join(''):'<div class="empty">No other players yet.</div>');
  }
  ...
  function appendMainControls(){
    ...
    if(state.status==='registration'){
      if(state.viewer.joined&&!state.viewer.isHost)controls.insertAdjacentHTML('beforeend',featureButton(state.viewer.ready?'NOT READY':'READY',state.viewer.ready?'unready':'ready',state.viewer.ready?'':'primary'));
      if(state.viewer.isHost)controls.insertAdjacentHTML('beforeend',featureButton(state.registrationLocked?'UNLOCK REGISTRATION':'LOCK REGISTRATION',state.registrationLocked?'unlock':'lock'));
    }
    if(state.status==='running'&&state.viewer.isHost)controls.insertAdjacentHTML('beforeend',featureButton(state.paused?'RESUME ARENA':'PAUSE ARENA',state.paused?'resume':'pause',state.paused?'primary':''));
    if(terminalStatus(state.status)&&state.viewer.isHost){
      if(!controls.querySelector('[data-action="newgame"]'))controls.insertAdjacentHTML('beforeend',button('START NEW GAME','newgame','primary','arena'));
      controls.insertAdjacentHTML('beforeend',featureButton('REMATCH','rematch','primary'));
      if(state.cooldownRemainingMs>0){controls.querySelectorAll('[data-action="newgame"],[data-feature-action="rematch"]').forEach(el=>el.disabled=true);controls.insertAdjacentHTML('beforeend','<div class="cooldown-note">Next Arena in '+featureEsc(state.cooldownText)+'</div>')}
    }
  }
  function terminalStatus(status){return ['finished','cancelled','aborted'].includes(String(status||''))}
  ...
  function renderFeaturePack(){
    ...
    appendMainControls(); renderHostTools(); renderHistory(); renderVoteProgress(); renderRecap(); renderReactions(); renderDiagnostics();
