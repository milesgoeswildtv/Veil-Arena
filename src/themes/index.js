...
const BASE_THEME = {
  id: "base", displayName: "Arena", tone: "chaotic",
...
const THEMES = new Map([[BASE_THEME.id, BASE_THEME], [VQS_THEME.id, VQS_THEME], [FULL_TILT_GAMBA.id, FULL_TILT_GAMBA]]);
...
export function chooseNarration(theme, poolName, values, rng = Math.random, recent = []) { const pool = theme?.[poolName] || BASE_THEME[poolName] || []; if (!pool.length) return { template: null, text: "The Arena makes its choice." }; const blocked = new Set(recent.slice(-100)); const candidates = pool.filter(line => !blocked.has(line)); const source = candidates.length ? candidates : pool; const template = source[Math.floor(rng() * source.length)]; return { template, text: renderTemplate(template, values) }; }
