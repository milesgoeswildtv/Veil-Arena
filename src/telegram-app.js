...
const last=state.lastEvent?.text||state.displayLog?.at(-1)?.text|| (state.status==='registration'?'Players are entering the Arena.':state.status==='running'?'Arena is moving…':'No event yet.');$('event').textContent=last;
