export const ACTIVITY_LIVE_CLIENT = String.raw`import { DiscordSDK } from "/activity/sdk.js";
...
function button(label, action, cls = "", payload = {}) {
  const b = document.createElement("button"); b.className = `btn ${cls}`.trim(); b.textContent = label; b.disabled = busy;
  b.addEventListener("click", () => act(action, payload)); return b;
}
...
function showFx(kicker, title, sub, duration = 4200) {
  if (!ui.overlay) return; text(ui.fxKicker, kicker); text(ui.fxTitle, title); text(ui.fxSub, sub); ui.overlay.classList.add("show");
  if (fxTimer) clearTimeout(fxTimer); if (duration) fxTimer = setTimeout(closeFx, duration);
}
...
async function api(path, init = {}) {
  const headers = new Headers(init.headers || {}); if (sessionToken) headers.set("x-arena-session", sessionToken);
...
async function setupDiscord() {
  ...
  const token = await fetch("/activity/oauth/token", {
    method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ code, guildId, channelId })
  });
  ...
  } catch { text(ui.location, "VOICE CHANNEL // ARENA"); }
  await refresh();
...
function maybeFx(state) {
  ...
  if (state.status === "finished" && state.winnerName) {
    ...
    if (lastFxKey !== key) { lastFxKey = key; showFx("🏆 ARENA COMPLETE", `${state.winnerName} WINS`, state.payoutReport || "One player is left.", 0); }
    return;
  }
  if (!latest?.at) return;
  const key = `${latest.at}:${latest.text}`; if (key === lastFxKey) return; lastFxKey = key;
  const msg = String(latest.text || ""); const upper = msg.toUpperCase();
  if (upper.includes("MASS BRAWL")) { ui.stage?.classList.add("lockdown"); showFx("⚠ ARENA OVERRIDE", "MASS BRAWL", msg, 5000); setTimeout(() => ui.stage?.classList.remove("lockdown"), 5200); }
  else if (upper.includes("REVIVAL") || upper.includes("SECOND CHANCE")) showFx("⚡ RECOVERY PROTOCOL", "SECOND CHANCE", msg, 4300);
  else if (upper.includes("FINAL BET") || upper.includes("FINAL SCARE") || upper.includes("CROWD PIN") || upper.includes("COMMUNITY SHOWDOWN")) showFx("👁 CROWD DECISION", "SHOWDOWN", msg, 4200);
  else if (state.aliveCount === 5) showFx("☠ ENDGAME", "FINAL FIVE", "Special protocols are disabled. Nobody is coming to save you.", 3500);
}
...
function renderVote(state) {
  ...
  for (const p of state.players.filter(x => x.alive)) {
    ...
    const score = document.createElement("span"); score.textContent = String(vote.totals?.[p.id] || 0); b.appendChild(score); ui.vote.appendChild(b);
  }
}
...
function renderActions(state) {
  ...
  if (state.status === "registration") {
    ...
    } else setNotice(state.viewer.joined ? "You are entered. Waiting for the host to start." : "Registration is open. Enter from this Activity window.");
    return;
  }
...
function render(state) {
  current = state;
  const theme = themeClass(state); text(ui.brand, theme.brand); text(ui.title, theme.title);
  text(ui.playerCount, state.playerCount || 0); text(ui.aliveCount, state.aliveCount || 0);
  text(ui.system, state.status ? String(state.status).toUpperCase() : "READY");
  if (!state.gameId) { text(ui.round, "ARENA READY"); text(ui.headline, "OPEN THE ARENA"); text(ui.event, "Launch a real Arena for this voice channel. Everyone in the Activity can enter from here."); }
  else if (state.status === "registration") { text(ui.round, "REGISTRATION"); text(ui.headline, "THE DOORS ARE OPEN"); text(ui.event, `${state.playerCount} player${state.playerCount === 1 ? "" : "s"} entered. Host starts when ready.`); }
  else if (state.status === "running") { text(ui.round, `ROUND ${state.round || 0}`); text(ui.headline, state.crowdVote?.open ? "THE CROWD HAS CONTROL" : state.aliveCount === 5 ? "FINAL FIVE" : "THE ARENA IS LIVE"); text(ui.event, state.latestDisplay?.text || "The next outcome is loading…"); }
  else { text(ui.round, `MATCH COMPLETE // ${state.round || 0} ROUNDS`); text(ui.headline, state.winnerName ? `${state.winnerName} WINS` : "ARENA COMPLETE"); text(ui.event, state.payoutReport || state.latestDisplay?.text || "The Arena is over."); }
  renderRoster(state); renderActions(state); renderVote(state); maybeFx(state);
}

async function refresh() {
  if (!sessionToken || busy) return;
  try { const data = await api("/activity/state"); render(data.state); }
  catch (error) { setNotice(error.message, true); text(ui.connection, "CONNECTION ERROR"); }
}

async function act(action, extra = {}) {
  if (busy) return; busy = true; renderActions(current || { gameId: null });
  try {
    const data = await api("/activity/action", { method: "POST", body: JSON.stringify({ action, ...extra }) });
    if (data.message) setNotice(data.message); if (data.state) render(data.state); else await refresh();
  } catch (error) { setNotice(error.message, true); }
  finally { busy = false; if (current) renderActions(current); }
}

window.addEventListener("error", event => { text(ui.connection, "ACTIVITY ERROR"); setNotice(event.message || "Activity error", true); });
window.addEventListener("unhandledrejection", event => { text(ui.connection, "ACTIVITY ERROR"); setNotice(event.reason?.message || String(event.reason || "Activity error"), true); });

setupDiscord().catch(error => {
  text(ui.connection, "SETUP REQUIRED"); text(ui.system, "AUTH FAILED"); text(ui.headline, "ACTIVITY NEEDS ONE SETUP FIX");
  text(ui.event, error.message); setNotice(error.message, true); clearActions();
});
`;
