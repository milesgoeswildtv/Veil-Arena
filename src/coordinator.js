...
function recentTemplates(game) { return game.history.filter(item => item?.narrationTemplate).slice(-100).map(item => item.narrationTemplate); }
...
function genericNormalNarration(game, result) {
  const ids = result.actorIds || []; const a = ids[0] ? tag(game, ids[0]) : "**SOMEONE**"; const b = ids[1] ? tag(game, ids[1]) : "**SOMEONE**"; const c = ids[2] ? tag(game, ids[2]) : "**SOMEONE**";
  const map = {
...
function renderBeat(game, theme, result, number) {
  if (!result.eliminatedIds?.length) return `**${number}.** ${genericNormalNarration(game, result)}`;
  const victimId = result.eliminatedIds[0]; const victim = name(game, victimId);
  if (result.type === "self_elimination") {
    const picked = chooseNarration(theme, "selfKills", { victim }, Math.random, recentTemplates(game)); rememberNarration(game, picked.template);
    return `**${number}. 💀 SELF-ELIMINATION — ${tag(game, victimId)}**\n${picked.text}\n➡️ **ELIMINATED: ${victim.toUpperCase()}**`;
  }
  const attackerId = result.actorIds.find(id => id !== victimId) || result.actorIds[0]; const killer = name(game, attackerId); const thirdId = result.actorIds.find(id => id !== victimId && id !== attackerId);
  const picked = chooseNarration(theme, "playerKills", { killer, victim, third: thirdId ? name(game, thirdId) : "someone else" }, Math.random, recentTemplates(game)); rememberNarration(game, picked.template);
...
function renderRevival(game, theme, result) {
  if (result.type === "revival_skipped") return `# 🕯️ ═══ ${theme.labels.revival} ═══ 🕯️\n**REVIVAL EVENT**\n\nThe Veil stirs, but there aren't two eliminated contestants available.`;
  const winner = name(game, result.winnerId); const loser = name(game, result.loserId);
  const picked = chooseNarration(theme, "revivalDuels", { winner, loser }, Math.random, recentTemplates(game)); rememberNarration(game, picked.template);
...
function renderCrowdResolution(game, theme, result, simulatedVotes = 0) {
  const winner = name(game, result.survivorId); const losers = result.eliminatedIds.map(id => name(game, id)); const pool = result.qualifiers.length > 2 ? "multiPins" : "pinDuels";
  const picked = chooseNarration(theme, pool, { winner, loser: losers[0] || "someone" }, Math.random, recentTemplates(game)); rememberNarration(game, picked.template);
...
export class ArenaCoordinator {
  ...
  async fetch(request) { const body = await request.json().catch(() => ({})); if (body.channelId) await this.ctx.storage.put("channelId", body.channelId); const channelId = body.channelId || await this.ctx.storage.get("channelId"); if (!channelId) return new Response("Missing channelId", { status: 400 }); if (body.action === "kick") { await this.ctx.storage.setAlarm(Date.now() + 1000); return Response.json({ ok: true }); } if (body.action === "wake") { await this.ctx.storage.setAlarm(Date.now() + 250); return Response.json({ ok: true }); } return Response.json({ ok: true, channelId }); }

  async alarm() {
    const channelId = await this.ctx.storage.get("channelId"); if (!channelId || !this.env.DB || !this.env.DISCORD_BOT_TOKEN) return;
    const game = await loadActiveGameForChannel(this.env.DB, channelId); if (!game || game.status !== "running") return; const theme = getTheme(game.themeId);
    if (game.crowdVote?.status === "open") {
      const simulatedVotes = castSimulatedCrowdVotes(game); const result = resolveCrowdVote(game); await saveGame(this.env.DB, game);
      ...
      if (await finishIfNeeded(this.env, game, theme)) return; await this.ctx.storage.setAlarm(Date.now() + NORMAL_DELAY_MS); return;
    }
    const { round, phases } = beginNextRound(game);
    if (!phases.length) { if (game.status === "finished") { await saveGame(this.env.DB, game); await recordFinishedGame(this.env.DB, game); await createChannelMessage(channelId, this.env.DISCORD_BOT_TOKEN, { content: winnerMessage(game, theme) }); } return; }
    ...
    if (phases.includes("revival") && specialEventsEnabled(game)) {
      const revival = resolveRevivalPit(game); await createChannelMessage(channelId, this.env.DISCORD_BOT_TOKEN, { content: renderRevival(game, theme, revival) + remaining(game) });
    } else if (phases.includes("crowd_vote") && specialEventsEnabled(game)) {
      const vote = openCrowdVote(game);
      if (vote) { await saveGame(this.env.DB, game); const simLine = game.testMode?.simulatedCrowd ? "\n\n🧪 **TEST MODE:** simulated spectators will also vote when time expires." : ""; await createChannelMessage(channelId, this.env.DISCORD_BOT_TOKEN, { content: `# 👁️ ═══ ${theme.labels.crowdVote} ═══ 👁️\n## THE FINAL SCARE IS OPEN\n\nSpectators have **30 seconds**. Choose who gets thrown into the Final Scare.\n\nTop two voting positions enter. A cutoff tie drags everyone tied into the fight. **ONE SURVIVES.**${simLine}`, components: [{ type: 1, components: [{ type: 2, style: 4, custom_id: `arena:vote_open:${game.id}:0`, label: "CAST YOUR VOTE", emoji: { name: "👁️" } }] }] }); await this.ctx.storage.setAlarm(Date.now() + CROWD_VOTE_MS); return; }
    } else {
    ...
    await saveGame(this.env.DB, game); await this.ctx.storage.setAlarm(Date.now() + NORMAL_DELAY_MS);
  }
}
