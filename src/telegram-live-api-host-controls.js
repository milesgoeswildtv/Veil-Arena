...
async function resolveLatestGame(db, launchGameId) {
  let game = await loadGame(db, launchGameId);
  const seen = new Set();
  for (let hop = 0; game?.telegramSuccessorGameId && hop < MAX_SUCCESSOR_HOPS; hop++) {
    if (seen.has(game.id)) break;
    seen.add(game.id);
    const next = await loadGame(db, game.telegramSuccessorGameId);
    if (!next) break;
    game = next;
  }
  return game;
...
async function authenticate(request, env) {
  ...
  const launchGameId = gameIdFromStartParam(auth.startParam);
  if (!launchGameId) throw new Error("This Telegram launch does not contain an Arena ID.");
  let game = await resolveLatestGame(env.DB, launchGameId);
...
function assertHost(game, user, action) {
  if (String(user.id) !== String(game.hostId)) {
    throw new Error(`Only the Arena host can ${action}.`);
  }
}
...
async function hostBots(env, game, user, mode) {
  ...
  assertHost(game, user, "use test controls");
...
async function startRegistration(env, game, user) {
  assertHost(game, user, "start Arena");
...
async function resetArena(env, game, user) {
  assertHost(game, user, "reset Arena");
...
async function forceCloseArena(env, game, user) {
  assertHost(game, user, "force-close Arena");
  if (game.status === "finished") throw new Error("Arena is already finished. Start a new game instead.");
  ...
  game.lastEvent = {
    type: "force_closed",
    round: Number(game.round) || 0,
    text: "Arena was force-closed by the host.",
    at: new Date().toISOString()
  };
  if (!Array.isArray(game.history)) game.history = [];
  game.history.push({ type: "force_closed", byUserId: String(user.id), round: Number(game.round) || 0, at: game.lastEvent.at });
  await saveGame(env.DB, game);
  await env.DB.prepare("DELETE FROM arena_registrations WHERE game_id = ?").bind(game.id).run();
  return game;
}

async function startNewArena(env, game, user) {
  assertHost(game, user, "start a new game");
  if (!["finished", "cancelled"].includes(game.status)) {
    throw new Error("Finish or force-close the current Arena before starting a new game.");
  }
...
export async function telegramAction(request, env) {
  ...
  else if (action === "forceclose") updated = await forceCloseArena(env, game, auth.user);
  else if (action === "newgame") updated = await startNewArena(env, game, auth.user);
  else if (action === "vote") {
