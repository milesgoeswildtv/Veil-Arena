...
body[data-arena-phase="registration"] #hero .controls .primary{grid-column:span 1}
...
body[data-arena-phase="finished"][data-arena-view="arena"] .sponsorship-card,
body[data-arena-phase="finished"][data-arena-view="arena"] .footer-row{display:none!important}
...
body[data-arena-view="more"] #hostTools,body[data-arena-view="more"] #recapPanel{display:none!important}
.more-intro{display:none;margin:0 0 13px;padding:18px;border:1px solid #493257;border-radius:15px;background:linear-gradient(145deg,#120c18,#09070d)}body[data-arena-view="more"] .more-intro{display:block}.more-intro h2{margin:0;font-size:25px}.more-intro p{margin:7px 0 0;color:#96889f;font-size:11px;line-height:1.45}.feature-grid.product-more{grid-template-columns:1fr!important;gap:9px!important}.feature-grid.product-more .feature-card{padding:0!important;overflow:hidden}.feature-grid.product-more .feature-card summary{padding:15px!important}.feature-grid.product-more .feature-card .feature-body{margin:0!important;padding:0 15px 15px}.feature-grid.product-more .feature-card:not(details){padding:15px!important}.history-entry.product-history{position:relative;padding:10px 0 10px 30px!important}.history-entry.product-history:before{content:"";position:absolute;left:9px;top:17px;width:7px;height:7px;border-radius:50%;background:#8455a2;box-shadow:0 0 10px #8455a266}.history-entry.product-history[data-kind="elimination"]:before{background:#d54f69}.history-entry.product-history[data-kind="revival"]:before{background:#4ddba8}.history-entry.product-history[data-kind="showdown"]:before{background:#c777ff}.history-entry.product-history[data-kind="winner"]:before{background:#f2c968}
...
(() => {
  ...
  function phase(){
    ...
    if(state.status==='registration')return 'registration';
    if(state.status==='running')return 'running';
    if(state.status==='finished')return 'finished';
    return String(state.status||'other');
  }
  ...
  function renderPlayerRows(){
    ...
    rows.forEach((row,index)=>{
      ...
      row.dataset.productPlayerId=String(p.id);
      row.setAttribute('role','button');row.setAttribute('tabindex','0');
      row.setAttribute('aria-label','View '+String(p.displayName||'player')+' stats');
    });
  }
